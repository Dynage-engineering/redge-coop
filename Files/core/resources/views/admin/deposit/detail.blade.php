    ...
    <div class="row gy-4 justify-content-center">
        <div class="col-xl-4 col-md-6">
            <div class="card b-radius--10 overflow-hidden box--shadow1">
                <div class="card-header">
                    @if ($deposit->branch)
                        <h5 class="card-title">@lang('Deposited from') {{ __(@$deposit->branch->name) }}</h5>
                    @else
                        <h5 class="card-title">@lang('Deposited Via') {{ __(@$deposit->gateway->name) }}</h5>
                    @endif
                </div>
                <div class="card-body">
                    <ul class="list-group list-group-flush">
                        ...
                        <li class="list-group-item border-0 d-flex justify-content-between align-items-center">
                            @lang('Username')
                            <span class="fw-bold"><a href="{{ route('admin.users.detail', $deposit->user_id) }}">{{ @$deposit->user->username }}</a></span>
                        </li>
                        ...
                        <li class="list-group-item border-0 d-flex justify-content-between align-items-center">
                            @lang('Amount')
                            <h5 class="fw-bold">{{ showAmount($deposit->amount) }} {{ __($general->cur_text) }}</h5>
                        </li>
        ...
        @if ($details || $deposit->status == Status::PAYMENT_PENDING)
            <div class="col-xl-8 col-md-6 mb-30">
                <div class="card b-radius--10 overflow-hidden box--shadow1">
                    <div class="card-header">
                        <h5 class="card-title">@lang('User\'s Information')</h5>
                    </div>
                    <div class="card-body">
                        @if ($details != null)
                        ...
                        @if ($deposit->status == Status::PAYMENT_PENDING)
                            <div class="row mt-4">
                                <div class="col-md-12">
                                    <button class="btn btn-outline--success ms-1 confirmationBtn" data-action="{{ route('admin.deposit.approve', $deposit->id) }}" data-question="@lang('Are you sure to approve this transaction?')"><i class="la la-check"></i>
                                        @lang('Approve')
                                    </button>

                                    <button class="btn btn-outline--danger ms-1 rejectBtn" data-id="{{ $deposit->id }}" data-info="{{ $details }}" data-amount="{{ showAmount($deposit->amount) }} {{ __($general->cur_text) }}" data-username="{{ @$deposit->user->username }}"><i class="la la-ban"></i> @lang('Reject')
                                    </button>
                                </div>
                            </div>
                        @endif
                ...
                <form action="{{ route('admin.deposit.reject') }}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label class="mt-2">@lang('Reason of Rejection')</label>
